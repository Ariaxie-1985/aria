...
def test_interviewTemplate_create_update(b_login_app):
    r = interviewTemplate_create_update(userToken=b_login_app[0])
    global templateId
    templateId = r.get('content', 0)
    assert_equal(1, r['state'], '创建面试信息用例通过')


def test_interviewTemplate_all(b_login_app):
    r = interviewTemplate_all(userToken=b_login_app[0])
    global template_list
    template_list = [t['id'] for t in r['content']]
    assert_in(templateId, template_list, "添加面试信息用例通过")


def test_interviewTemplate_del(b_login_app):
    r = interviewTemplate_del(userToken=b_login_app[0], templateId=templateId)
    assert_in(templateId, template_list, "删除面试信息用例通过")


def test_interviewTemplate_all1(b_login_app):
    r = interviewTemplate_all(userToken=b_login_app[0])
    template_list = [t['id'] for t in r['content']]
