...
def test_orderResumes_query(b_login_app, resumeStageCode, ip_port):
    r = orderResumes_query(userToken=b_login_app[0], userId=b_login_app[1], resumeStageCode=resumeStageCode,
                           ip_port=ip_port)
...
def test_orderResumes_sameResume_query(b_login_app, ip_port):
    r = orderResumes_sameResume_query(userToken=b_login_app[0], userId=b_login_app[1], resumeId=orderResumeId,
                                      ip_port=ip_port)
    global order_resumeId, positionId
    for result in r['content']['result']:
        if result['resumeStage'] == "OBSOLETE":
...
def test_orderResumes_stage_obsolete(b_login_app, ip_port):
    r = orderResumes_stage(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId, ip_port=ip_port)
    assert_equal("OBSOLETE", r['content']['stage'], '淘汰阶段用例通过')


def test_orderResumes_resume_new(b_login_app, ip_port):
    r = orderResumes_resume_new(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId,
                                ip_port=ip_port)
    assert_equal(1, r['state'], '将淘汰简历重新恢复为候选人用例通过')


def test_orderResumes_detail(b_login_app, ip_port):
    r = orderResumes_detail(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId, ip_port=ip_port)
    assert_equal(str(order_resumeId), r['content']['orderResumeId'], "查询简历详情用例通过")


def test_orderResumes_read(b_login_app, ip_port):
    r = orderResumes_read(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId, ip_port=ip_port)
    assert_equal(1, r['state'], '设置简历已读用例通过')


def test_orderResumes_resume_link(b_login_app, ip_port):
    r = orderResumes_resume_link(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId,
                                 ip_port=ip_port)
...
def test_orderResumes_process_query(b_login_app, ip_port):
    r = orderResumes_process_query(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId,
                                   ip_port=ip_port)
    assert_equal(1, r.get('state', 0), "查询简历参与者的评价记录")


def test_orderResumes_resume_interview(b_login_app, ip_port):
    r = orderResumes_resume_interview(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId,
                                      positionId=positionId, ip_port=ip_port)
    global resume_state
...
def test_orderResumes_interview_datetime(b_login_app, ip_port):
    r = orderResumes_interview_datetime(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId,
                                        ip_port=ip_port)
...
def test_orderResumes_resume_luyong(b_login_app, ip_port):
    r = orderResumes_resume_luyong(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId,
                                   ip_port=ip_port)
...
def test_orderResumes_resume_employed(b_login_app, ip_port):
    r = orderResumes_resume_employed(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId,
                                     ip_port=ip_port)
...
def test_orderResumes_resume_obsolete(b_login_app, ip_port):
    r = orderResumes_resume_obsolete(userToken=b_login_app[0], userId=b_login_app[1], resumeId=order_resumeId,
                                     ip_port=ip_port)
