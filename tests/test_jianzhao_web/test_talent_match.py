...
#推荐人才页面验证
@pytest.mark.parametrize ("positionid", [("7305248")])
...
def test_collection(positionid):
    ...
    assert_equal (1, get_res.get('state'), "收藏成功",f"收藏失败，失败原因:{get_res.get('message')}")
...
def test_uncollection():
    ...
    a = get_res.get('state')
    assert_equal(1, get_res.get('state'), "取消收藏成功", f"取消收藏失败，失败原因:{get_res.get('message')}")

# 上下翻页验证
@pytest.mark.parametrize("positionId", [('7305248')])
def test_pages(positionId):
    get_res = talent_pages(positionId)
    assert_equal(1, get_res.get('state'), "翻页成功", f"翻页失败，失败原因:{get_res.get('message')}")
