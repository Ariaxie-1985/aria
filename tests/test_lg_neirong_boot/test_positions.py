...
def test_get_online_positions(login_app):
    r = get_online_positions(userToken=login_app[0], userId=login_app[1], H9=True)
    global positionId
...
def test_positions_mark_info(login_app, ip_port):
    r = positions_mark_info(userToken=login_app[0], userId=login_app[1], positionId=positionId, ip_port=ip_port)
