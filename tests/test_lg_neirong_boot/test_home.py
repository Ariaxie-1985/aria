...
def test_deliver_rec(login_app, ip_port):
    r = deliver_rec(userToken=login_app[0],userId=login_app[1], ip_port=ip_port)
    assert_equal(True, bool(r['content']), '大家都在投用例通过')


def test_fast_feedback(login_app, ip_port):
    r = fast_feedback(userToken=login_app[0],userId=login_app[1],ip_port=ip_port)
    assert_equal(True, bool(r['content']), '极速反馈用例通过')


def test_home_headline(login_app, ip_port):
    r = home_headline(userToken=login_app[0],userId=login_app[1],ip_port=ip_port)
