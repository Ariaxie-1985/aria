...
def test_password_login():
    ...
    assert_equal(expect_value=1, actual_value=r['state'], success_message='校验密码登录成功', fail_message='校验密码登录失败', te='王洋')
...
def test_send_verify_code():
    ...
    assert_equal(expect_value=1, actual_value=r.get('state'), success_message='校验发送验证码成功', fail_message="校验发送验证码失败", te='王洋')
