...
def test_is_enough_positions(b_login_app):
    ...
    assert_equal(1, positions_result['state'], '确认在线职位数是否足够发布职位用例通过', te='foxtang')
...
class TestJd(object):
    ...
    def test_publish_position(self, b_login_app):
        ...
        assert_equal(1, r.get('state'), "校验发布职位成功", te='foxtang')
    ...
    def test_talent_recTalent(self, b_login_app):
        ...
        assert_equal(True, bool(len(r['content']['result'])), "推荐人才用例通过", te='foxtang')
        global resumeId
    ...
    def test_talent_newTalent(self, b_login_app):
        ...
        assert_equal(True, bool(len(r['content']['result'])), "最新人才用例通过", te='foxtang')
        for talent in r['content']['result']:
            if bool(talent.get('portrait', False)):
                assert_in(expect_value=talent['portrait'].split(".")[-1], actual_value=self.portrait_format,
                          success_message="最新人才的头像信息用例通过", te='foxtang')

    def test_talent_collections(self, b_login_app):
        r = talent_collections(userToken=b_login_app[0])
        assert_equal(True, bool(len(r['content']['result'])), "人才收藏用例通过", te='foxtang')
        for talent in r['content']['result']:
            if bool(talent.get('portrait', False)):
                portrait = json.loads(talent['portrait'])
                assert_in(portrait['url'].split(".")[-1], self.portrait_format, "人才收藏的头像信息用例通过", te='foxtang')
    ...
    def test_recommend_isExistPositionList(self, c_login_app):
        ...
        assert_equal(1, r.get('state'), '投递后推荐的职位 （投了又投），是否有数据用例通过', te='foxtang')
    ...
    def test_recommend_positionList(self, c_login_app):
        ...
        assert_equal(1, r.get('state'), '投递后推荐的职位 （投了又投)用例通过', te='foxtang')
    ...
    def test_session_share_info_app_b_yun(self, b_login_app):
        ...
        assert_equal(1, r.get('state'), 'B端用户与C端用户的消息里分享简历到微信好友用例通过', te='foxtang')

    def test_quickReply_all(self, b_login_app):
        r = quickReply_all(userToken=b_login_app[0])
        assert_equal(1, r.get('state'), "获取IM的快捷回复用例通过", te='foxtang')
    ...
    def test_chat_c_position(self, b_login_app, c_login_app):
        ...
        assert_equal(False, r['content']['positionDelivered'], "获取IM在沟通的职位用例通过", te='foxtang')
    ...
    def test_chat_interview_check(self, b_login_app):
        ...
        assert_equal(1, interview_state, '检查候选人已淘汰是否确认邀约面试用例通过', te='foxtang')
    ...
    def test_orderResumes_resume_interview(self, b_login_app):
        ...
        assert_equal(1, r.get('state', 0), '邀约面试用例通过', te='foxtang')
    ...
    def test_orderResumes_resume_obsolete(self, b_login_app):
        ...
        assert_equal(1, r.get('state'), '候选人状态调整为不合适用例通过', te='foxtang')
