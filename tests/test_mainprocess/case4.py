...
def test_greeting_list():
    r = greeting_list(cUserIds=9722494, positionId=7986428)
    assert_equal(1, r['state'], '找人才-打招呼用例通过')


def test_multiChannel_default_invite():
    r = multiChannel_default_invite(positionId=7986428)
    assert_equal(1, r['state'], '职位邀请人才用例通过')


def test_session_batchCreate_cUserIds():
    r = session_batchCreate_cUserIds(cUserIds=9722494, positionId=7986428)
