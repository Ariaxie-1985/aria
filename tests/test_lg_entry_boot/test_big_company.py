...
def test_get_hot_company(login_app, city):
    r = get_hot_company(userToken=login_app, city=city)
...
def test_query_company_index(login_app):
    r = query_company_index(userToken=login_app, companyId=hot_companyId)
...
def test_query_all_company(login_app):
    r = query_all_company(userToken=login_app)
    assert_in(423911, r['content'], '查询所有大公司ID列表用例通过')


def test_query_urgent_positions(login_app):
    r = query_urgent_positions(userToken=login_app, companyId=hot_companyId)
    assert_equal(1, r['state'], '查询所有大公司ID列表用例通过')


def test_is_big_company(login_app):
    r = is_big_company(userToken=login_app, companyId=423911)
    assert_equal(True, r['content'], '是否大公司用例通过')


def test_get_company_questions(login_app):
    r = get_company_questions(userToken=login_app, companyId=hot_companyId)
    assert_equal(True, bool(r['content']['questionV2s']), '公司问答列表查询用例通过')


def test_company_attention_add(login_app):
    r = company_attention_add(userToken=login_app, companyId=hot_companyId)
    assert_equal(1, r['state'], '关注公司用例通过')


def test_company_attention_list(login_app):
    r = company_attention_list(userToken=login_app)
    assert_equal(hot_companyId, r['content']['attentionCompanies'][0]['companyId'], '验证关注公司是否成功用例通过')


def test_company_attention_delete(login_app):
    r = company_attention_delete(userToken=login_app, companyId=hot_companyId)
    assert_equal(1, r['state'], '查询关注的公司列表用例通过')


def test_company_attention_list1(login_app):
    r = company_attention_list(userToken=login_app)
    assert_equal(False, bool(r['content']['attentionCompanies']), '验证取消关注公司是否成功用例通过')


@pytest.mark.parametrize('positionType', [(0), (1), (2)])
def test_query_interview_experience(login_app, positionType):
    r = query_interview_experience(userToken=login_app, companyId=hot_companyId, positionType=positionType)
    assert_equal(True, bool(r['content']['interviewScore']), '查询面试评价用例通过')


def test_query_company_score(login_app):
    r = query_company_score(userToken=login_app, companyId=hot_companyId)
