...
def test_get_hr_info(login_app):
    r = get_hr_info(userToken=login_app, publisherId=15130154)
    assert_equal(418937, r['content']['companyId'], '获取HR信息用例通过')


def test_get_hr_card(login_app):
    r = get_hr_card(userToken=login_app, publisherId=15130154)
    assert_equal(15130154, r['content'][0]['hrId'], '获取HR卡片信息用例通过')


def test_get_info(login_app):
    r = get_info(userToken=login_app, publisherId=15130154)
    assert_equal("拉勾测试简称XM", r['content']['company'], '获取HR信息用例通过')


def test_get_baseStatus(login_app):
    r = get_baseStatus(userToken=login_app, publisherId=15130154)
