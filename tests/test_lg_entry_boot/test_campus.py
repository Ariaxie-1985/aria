...
def test_get_user_info(login_app):
    r = get_user_info(userToken=login_app)
    assert_equal(1, r['state'], "校招--获取用户信息用例通过")


def test_get_campus_count(login_app):
    r = get_campus_count(userToken=login_app,companyId=companyId)
