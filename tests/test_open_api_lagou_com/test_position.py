...
def test_category_get(get_access_token):
...
def test_company_address_district(get_access_token):
...
def test_company_address_create(get_access_token, get_openid):
...
def test_address_query(get_access_token):
...
def test_company_address_list(get_access_token):
...
def test_position_create(get_access_token, get_openid):
...
def test_get_create_position_info(get_access_token):
    ...
    assert_equal(position_id, res['data']['position']['position_id'], '查询创建的职位信息用例通过', f'查询创建的职位{position_id}信息用例失败')


def test_update_position_info(get_access_token):
...
def test_get_update_position_info(get_access_token):
    time.sleep(1)
    res = get_position_info(access_token=get_access_token, position_id=position_id)
    assert_equal(0, res.get('code'), '更改职位信息请求成功', '更改职位信息请求失败')
...
def test_get_position_list(get_access_token, status):
    time.sleep(1)
...
def test_offline_position(get_access_token):
...
def test_publish_position(get_access_token):
...
def test_refresh_position(get_access_token):
...
def test_delete_position_address(get_access_token):
