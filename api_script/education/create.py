...
def create_shop_goodsOrder_course(payLagouCoinNum, sellGoodsPriceId, gateLoginToken,shopOrderToken):
    ...
    header.add({'X-L-REQ-HEADER': json.dumps({'deviceType': 1}), "Cookie": f"gate_login_token ={gateLoginToken};", 'shop-order-token': shopOrderToken})
    data = {
    ...
    remark = '课程订单创建'
    return json_post(url=url, headers=header, data=data, remark=remark,re='程宇伟')
