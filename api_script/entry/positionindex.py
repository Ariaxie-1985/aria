...
def expect_job_list(userToken, ip_port=None, userId=None):
    ...
    header = app_header_999(userToken=userToken, userId=userId)
    return get_requests(url=url, headers=header, remark='求职意向', ip_port=ip_port).json()
...
def switch_city(userToken, city):
    ...
    return get_requests(url=url, headers=header, remark="切换搜索的职位").json()
...
def position_index_suggestion(userToken, userId=None, ip_port=None):
    ...
    return get_requests(url=url, headers=header, remark="个性化搜索", ip_port=ip_port).json()
