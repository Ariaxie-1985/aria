...
def get_info(userToken):
    ...
    return get_requests(url=url, headers=header, remark="查询app端配置").json()


def get_app_theme(userToken):
    url = 'https://gate.lagou.com/v1/entry/config/appSetting/getAppTheme'
    header = app_header_999(userToken, DA=False)
    return get_requests(url=url, headers=header, remark="查询app端主题").json()


def get_im_entrance(userToken):
    url = 'https://gate.lagou.com/v1/entry/config/appSetting/getIMEntrance'
    header = app_header_999(userToken, DA=False)
    return get_requests(url=url, headers=header, remark="查询APP消息页活动入口").json()
