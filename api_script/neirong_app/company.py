...
def create_benefit(userToken, id):
...
def benefit_category_left(userToken):
    url = "https://gate.lagou.com/v1/neirong/company/benefit/category/left"
    header = app_header_999(userToken, DA=False)
    return get_requests(url=url, headers=header, remark="查询公司该城市下未添加的的福利标签数据").json()


def get_benefit_baseInfo(userToken, companyId):
    url = "https://gate.lagou.com/v1/neirong/company/benefit/baseInfo?companyId={}".format(companyId)
    header = app_header_999(userToken, DA=False)
    return get_requests(url=url, headers=header, remark="查看公司福利信息").json()


def delete_benefit(userToken, id):
    url = "https://gate.lagou.com/v1/neirong/company/benefit/delete"
    header = app_header_999(userToken, DA=False)
    data = {
        "id": id  # 这里的id是 函数get_benefit_baseInfo返回的id
    }
    return json_post(url=url, headers=header, data=data, remark="删除公司福利")


def company_baseInfo(userToken, companyId):
...
def company_culture(userToken, companyId):
...
def company_detail(userToken, companyId):
...
def company_hasBenefit(userToken, companyId):
...
def company_question(userToken, companyId):
