...
def update_position_info(access_token, **kwargs):
    ...
    position_nature = kwargs.get('position_nature', '全职')
    work_year = kwargs.get('work_year', '3-5年')
    education = kwargs.get('education', '本科')
    advantage = kwargs.get('advantage', '20薪')
    detail = kwargs.get('detail', '肯加班拿钱少肯加班拿钱少肯加班拿钱少肯加班拿钱少肯加班拿钱少肯加班拿钱少肯加班拿钱少')
    url = f'https://open.lagou.com/v1/position/update?access_token={access_token}'
    data = {'position_id': position_id, 'work_year': work_year, 'position_nature': position_nature,
